...
var getCgroupManager = func(vmi *v1.VirtualMachineInstance, host string, hypervisorDeviceMinorNumber int64) (cgroup.Manager, error) {
	return cgroup.NewManagerFromVM(vmi, host, hypervisorDeviceMinorNumber)
}
...
func NewVirtualMachineController(
	...
	c.deviceManagerController = deviceManager.NewDeviceController(
		...
		maxDevices,
		permissions,
		deviceManager.PermanentHostDevicePlugins(baseCtrl.hypervisorRuntime.GetHypervisorDevice(), maxDevices, permissions),
		clusterConfig,
		nodeStore)
...
func (c *VirtualMachineController) processVmCleanup(vmi *v1.VirtualMachineInstance) error {
	...
	cgroupManager, _ := getCgroupManager(vmi, c.host, hypervisor.NewLauncherResourceRenderer(c.clusterConfig.GetHypervisor().Name).GetHypervisorDeviceMinorNumber())
...
func (c *VirtualMachineController) adjustResources(vmi *v1.VirtualMachineInstance) error {
	err := c.hypervisorRuntime.AdjustResources(c.podIsolationDetector, vmi, c.clusterConfig.GetConfig())
	if err != nil {
