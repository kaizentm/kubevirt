...
type TemplateService struct {
	...
	virtLauncherResourceRenderer     hypervisor.LauncherResourceRenderer
}
...
func (t *TemplateService) renderLaunchManifest(vmi *v1.VirtualMachineInstance, imageIDs map[string]string, backendStoragePVCName string, tempPod bool) (*k8sv1.Pod, error) {
	...
	if tempPod {
		...
		command = []string{"/usr/bin/virt-launcher-monitor",
			...
			"--hypervisor", t.clusterConfig.GetHypervisor().Name, // TODO This might need to be removed because virt-launcher is no longer updated
		}
		if nonRoot {
...
func (t *TemplateService) newResourceRenderer(vmi *v1.VirtualMachineInstance, networkToResourceMap map[string]string) (*ResourceRenderer, error) {
	...
	hypervisorDeviceK8sResource := K8sDevicePrefix + "/" + t.virtLauncherResourceRenderer.GetHypervisorDevice()
	baseOptions := []ResourceRendererOption{
		WithEphemeralStorageRequest(),
		WithVirtualizationResources(getRequiredResources(vmi, hypervisorDeviceK8sResource, t.clusterConfig.AllowEmulation())),
	}
...
func NewTemplateService(launcherImage string,
	...
	svc := TemplateService{
		...
		virtLauncherResourceRenderer: virtLauncherResourceRenderer,
	}
...
func (t *TemplateService) VMIResourcePredicates(vmi *v1.VirtualMachineInstance, networkToResourceMap map[string]string) VMIResourcePredicates {
	...
	memoryOverhead := t.virtLauncherResourceRenderer.GetMemoryOverhead(vmi, vmiCPUArch, t.clusterConfig.GetConfig().AdditionalGuestMemoryOverheadRatio)
